...
describe("failure: recognition", () => {
  ...
  it("isDomain agrees with isDomainKind for every constructible failure", () => {
    ...
    // inferred to mirror it symmetrically. Flagged as an inference — the spec
    // never states isDomain's formula in words.
    for (const { kind, make } of ALL) {
      const f = make("x");
...
describe("failure: cause chain", () => {
  it("because copies every field except cause, and does not mutate the original", () => {
    ...
    expect(result.retryAfter).toBe(original.retryAfter);
  ...
  it(
    ...
    () => {
      const self = F.internal("self-referential");
      (self as { cause?: F.Failure }).cause = self;
  ...
  it(
    ...
    () => {
      const a = F.internal("a");
      const b = F.internal("b");
      (a as { cause?: F.Failure }).cause = b;
      (b as { cause?: F.Failure }).cause = a;
      let result: F.Failure[] | undefined;
      expect(() => {
        result = F.chain(a);
...
describe("result: combinators", () => {
  ...
  it("mapErr applies f to an Err's error and wraps the result in Err", () => {
    ...
    expect(calls).toBe(1);
    expect(mapped.ok).toBe(false);
    expect((mapped as any).error).toBe("boom");
  });
  ...
  it("mapErr does not call f on an Ok, and preserves its value", () => {
    ...
    expect(mapped.ok).toBe(true);
    expect((mapped as any).value).toBe(value);
  });
  ...
  it("unwrapOr returns the fallback for an Err and never throws", () => {
    expect(() => R.err(F.internal("x")).unwrapOr(42)).not.toThrow();
    expect(R.err(F.internal("x")).unwrapOr(42)).toBe(42);
  });
});
...
describe("result: serialization boundary", () => {
  ...
  it("fromJSON(ok.toJSON()) round-trips to an equivalent Ok", () => {
    ...
    const reconstructed: any = R.fromJSON(original.toJSON());
  ...
  it("fromJSON(err.toJSON()) round-trips to an equivalent Err", () => {
    ...
    expect(reconstructed.error).toEqual(original.error);
  });
});
