...
/** FNV-1a gives each id a repeatable initial position. Fixed iterations then
 *  keep the picture stable across renders instead of sampling Math.random. */
 ...
 *  known: this is O(iterations x n²), intended for bounded diagrams.
 *
...
export function concentric(input: GraphInput, rootId: string): Placement {
  ...
  for (const [ring, ids] of [...rings].sort((a, b) => a[0] - b[0])) {
    ...
    ids.forEach((id, i) => {
      ...
      out.set(id, {
        x: px(cx + Math.cos(a) * radius),
        y: px(cy + Math.sin(a) * radius * 0.82),
      });
    });
  }
  return applyPins(out, input.pins);
...
export function tiered(
  ...
  const keys = [...new Set([...(order ?? []), ...groups.keys()])].filter(
    (key) => groups.has(key),
  );
  ...
  keys.forEach((key, col) => {
    ...
    const x =
      columns === 1 ? width / 2 : 28 + (col * (width - 56)) / (columns - 1);
    ids.forEach((id, i) => {
...
export function columns(input: GraphInput): Placement {
  ...
  for (const [column, [, ids]] of [...byColumn]
    ...
    const x =
      cols === 1
        ? input.width / 2
        : 28 + (column * (input.width - 56)) / (cols - 1);
    ids.forEach((id, i) => {
      out.set(id, {
        x: px(x),
        y: px(((i + 1) * input.height) / (ids.length + 1)),
      });
    });
  }
  return applyPins(out, input.pins);
