...
const make = (
  ...
  Object.assign(to, { domain, range, ticks: (n = 5) => niceTicks(domain, n) });
...
export function diverging(
  ...
  const extent = Math.max(1e-9, ...values.map((v) => Math.abs(v - midpoint)));
  return {
    // -1 .. 0 .. 1
 ...
 *  boundary carries forward the fix from Overwatch's entity canvas. */
