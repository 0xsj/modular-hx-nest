...
export const ForceNetwork = (p: Base) => (
  <GraphFrame title="Force network" {...p} layout="cose" />
);

/** Position is an ANSWER: the ring is hops from the centre. The only layout
 *  here whose geometry can be read off. */
export const RadialNetwork = (
  p: Base & {
    rootId: string;
  },
) => <GraphFrame title="Radial network" {...p} layout="concentric" />;
 ...
 *  into a solid disc. */
export const CircularNetwork = (p: Base) => (
  <GraphFrame title="Circular network" {...p} layout="circular" curved />
);

/** Two classes, edges only between them. Shape carries which side, because a
 *  bipartite graph drawn in one shape is just a graph. */
export const BipartiteNetwork = (p: Base) => (
  <GraphFrame title="Bipartite network" {...p} layout="tiered" />
);
...
export const MultipartiteNetwork = (
  p: Base & {
    groupOrder?: readonly (string | number)[];
  },
) => <GraphFrame title="Multipartite network" {...p} layout="tiered" />;
 ...
 *  against. */
export const FlowNetwork = (p: Base) => (
  <GraphFrame title="Flow network" {...p} layout="columns" />
);
...
export const CliqueNetwork = (p: Base) => (
  <GraphFrame title="Clique network" {...p} layout="circular" labels />
);
 ...
 *  read as a structure. */
export const HairballNetwork = (p: Base) => (
  <GraphFrame title="Hairball network" {...p} layout="cose" labels={false} />
);
...
export const EnrichmentMap = (
  p: Base & {
    hulls: GraphFrameProps["hulls"];
  },
) => <GraphFrame title="Enrichment map" {...p} layout="cose" />;
...
export const CorrelationNetwork = (p: Base) => (
  <GraphFrame title="Correlation network" {...p} layout="cose" curved />
);
...
export const ModuleNetwork = (
  p: Base & {
    hulls: GraphFrameProps["hulls"];
  },
) => <GraphFrame title="Module network" {...p} layout="cose" />;
