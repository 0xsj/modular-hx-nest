...
describe("VisuallyHidden", () => {
  it("stays in the accessibility tree", () => {
    ...
    expect(
      ...
      "hidden removes it from the tree entirely",
    ).toBe(false);
...
describe("AccessibleIcon", () => {
  it("gives the glyph words, and hides the glyph", () => {
    const { container } = render(() => (
      <AccessibleIcon label="Degraded">
        <Check size={12} />
      </AccessibleIcon>
    ));
    expect(container.textContent, "the words are what the icon MEANS").toBe(
      "Degraded",
    );
    expect(
      container.querySelector("[aria-hidden='true']")!.querySelector("svg"),
    ).toBeTruthy();
  });
...
describe("Portal", () => {
  it("renders outside its parent box and inside the component tree", () => {
    const { container } = render(() => (
      <div id="host">
        <Portal>
          <span data-portalled>elsewhere</span>
        </Portal>
      </div>
    ));
    expect(
