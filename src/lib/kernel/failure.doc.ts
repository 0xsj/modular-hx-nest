 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * THIS DOCUMENT IS AN ORACLE — `flover-solid ADR 0001`.
 *
 * `src/lib/**` is where the spec-test barrier applies, and this file is the
 * specification a barriered writer works from. It was written before
 * `failure.ts` existed, which is the only property that lets a test derived
 * from it DISAGREE with the code.
 *
 * §5 is normative and numbered. Everything else is reasoning, and a test
 * derived from it is a test derived from prose.
 *
 * A module doc rather than a tier doc: `doc.ts` explains what `kernel` is for,
 * and this explains one module inside it. A module large enough to be its own
 * oracle gets its own `<name>.doc.ts`, because an oracle must be sanitised and
 * an oracle carrying an unrelated module's prose is material a writer has to
 * be told to ignore.
 * ─────────────────────────────────────────────────────────────────────────────
 *
 *
 ...
 *
 *       ### AMENDED after implementation — added because it was missing
 *
 *       F1 pins only the STRUCTURE of the two sets. A mutation that deleted
 *       `forbidden` from `TRANSPORT_KINDS` altogether left them disjoint, left
 *       the union exact, and survived every clause here — because every other
 *       claim quantifies over `FAILURE_KINDS`, and the mutant had simply made
 *       that set smaller.
 *
 *       That is the defect the split exists to prevent, undetected: a kind
 *       that silently leaves the transport set becomes narrowable, and a
 *       service may then fold away a failure it does not control.
 *
 *       Pinning membership makes adding a kind a deliberate act that fails a
 *       test, which is the intended cost. Nothing else in §5 moved.
 *
