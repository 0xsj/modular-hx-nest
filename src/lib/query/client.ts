...
import { asFailure, retryDelay } from "../kernel";
 ...
 *  honoured, a refusal is never retried because it is an answer, and a user's
 *  own cancellation is never retried because they asked for it.
 *
  ...
  return new QueryClient({
    defaultOptions: {
      queries: {
        staleTime: 60_000,
        retry: (attempt, error) =>
          attempt < 2 && retryDelay(asFailure(error), attempt) !== null,
        retryDelay: (attempt, error) =>
          retryDelay(asFailure(error), attempt) ?? 0,
